    ...
    <div className="rounded-3xl border border-white/5 bg-slate/60 p-6">
      <div className="flex flex-wrap items-center justify-between gap-4 mb-6">
        ...
        >
          ...
          <input name="services" placeholder="Services *" className="input col-span-2 md:col-span-3" required />
          <button
            type="submit"
            disabled={isPending}
            className="col-span-2 rounded-full bg-accent px-4 py-2 text-sm font-semibold text-black hover:bg-white disabled:opacity-40 active:scale-95 transition-transform"
          >
      ...
      <div className="overflow-x-auto">
        <table className="w-full text-sm">
          ...
          <tbody className="divide-y divide-white/5">
              ...
              <tr key={client.id}>
                <td className="py-3 pr-4">
                  ...
                  <div className="flex gap-2 mt-0.5">
                    {client.frameioLink && (
                      <a href={client.frameioLink} target="_blank" rel="noreferrer" className="text-xs text-accent hover:underline inline-flex items-center gap-1">
                        Frame.io <ExternalLink size={9} />
                      </a>
                    )}
                    {client.contractUrl && (
                      <a href={client.contractUrl} target="_blank" rel="noreferrer" className="text-xs text-white/40 hover:text-white inline-flex items-center gap-1">
                ...
                <td className="py-3 pr-4">
                  <span className={`rounded-full px-2 py-0.5 text-xs font-medium ${stageBadge[client.stage] ?? "bg-white/10 text-white/60"}`}>
                    {client.stage}
                  </span>
                </td>
                <td className="py-3 pr-4 text-white/60">{client.tier}</td>
                <td className="py-3 pr-4 text-white/60 max-w-[160px] truncate">{client.services}</td>
                <td className="py-3 pr-4 text-white font-medium">
                  {client.monthlyRetainer ? `$${client.monthlyRetainer.toLocaleString()}` : "—"}
                </td>
                <td className="py-3 pr-4 text-white/40 text-xs whitespace-nowrap">{fmtDate(client.startDate)}</td>
                <td className="py-3">
                  <div className="flex gap-2">
                    <button onClick={() => { setEditingClient(client); setShowDeliverables(false); }} className="text-white/40 hover:text-white active:scale-95">
                      <Pencil size={14} />
                    </button>
                    <button onClick={() => startTransition(() => deleteClient(client.id))} disabled={isPending} className="text-white/40 hover:text-red-400 disabled:opacity-40 active:scale-95">
                      <Trash2 size={14} />
                    </button>
                  </div>
                </td>
            ...
            {clients.length === 0 && (
              <tr><td colSpan={8} className="py-8 text-center text-white/30 text-sm">No clients yet — add one above.</td></tr>
            )}
          </tbody>
        </table>
      </div>
        ...
        <div className="fixed inset-0 z-50 flex items-start justify-center bg-black/70 backdrop-blur-sm p-4 overflow-y-auto">
          <div className="w-full max-w-xl rounded-3xl border border-white/10 bg-slate p-6 space-y-4 my-8">
            <div className="flex items-center justify-between">
              ...
              <button onClick={() => setEditingClient(null)} className="text-white/40 hover:text-white text-2xl leading-none">×</button>
            </div>
            <form
              action={(fd) => {
                startTransition(async () => {
            ...
            >
              <div className="grid grid-cols-2 gap-3">
                ...
                <input name="frameioLink" defaultValue={editingClient.frameioLink ?? ""} placeholder="Frame.io link" className="input" />
                <input name="contractUrl" defaultValue={editingClient.contractUrl ?? ""} placeholder="Contract URL" className="input col-span-2" />
                <div className="col-span-2 space-y-1">
                  <label className="text-xs text-white/40">Start Date</label>
                  <input name="startDate" type="date" defaultValue={toDateInput(editingClient.startDate)} className="input date-input w-full" />
                </div>
                <input name="services" defaultValue={editingClient.services} placeholder="Services" className="input col-span-2" />
              </div>
              <div className="flex gap-2">
                <button type="submit" disabled={isPending} className="flex-1 rounded-full bg-accent py-2 text-sm font-semibold text-black hover:bg-white disabled:opacity-40 active:scale-95">
                  {isPending ? "Saving..." : "Save"}
                </button>
                <button type="button" onClick={() => setEditingClient(null)} className="flex-1 rounded-full border border-white/20 py-2 text-sm hover:bg-white/5 active:scale-95">Cancel</button>
              </div>
            </form>
            <div className="border-t border-white/5 pt-4">
              <button onClick={() => setShowDeliverables((v) => !v)} className="text-sm text-white/60 hover:text-white flex items-center gap-2 active:scale-95">
